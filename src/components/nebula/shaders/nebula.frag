// nebula.frag - FBM based soft nebula shader (prototype)
...
void main() {
    ...
    vec2 p = uv * 0.6 + vec2(u_time * 0.02);
    float n = fbm(p * 1.2);
    float m = fbm(p * 0.6 + n * 0.5 + u_mouse * 0.002);
    float density = smoothstep(0.2, 0.7, n * 0.6 + m * 0.4);
    vec3 color = mix(u_colorA, u_colorB, density);
