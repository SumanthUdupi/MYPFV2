    ...
    <div className={`flex ${isLeft ? 'flex-row-reverse' : 'flex-row'} items-stretch w-full mb-16`}>
      <div className="w-1/2 flex-shrink-0">
        ...
        >
          ...
          <h3 className="font-display text-2xl text-accent mb-2">{item.role}</h3>
          ...
          <ul className={`list-none text-sm text-text/80 ${isLeft ? 'pl-4' : 'pr-4'}`}>
            {item.points.map((point: string, i: number) => (
              <li key={i} className="mb-2 relative pl-6">
                <span className="absolute left-0 top-1 w-2 h-2 bg-accent/50 transform rotate-45"/>
                {point}
              </li>
            ))}
          </ul>
...
const Experience: React.FC = () => {
  ...
  return (
    <section id="experience" className="py-32">
      <h2 className="font-display text-6xl text-accent text-center mb-6">Experience</h2>
